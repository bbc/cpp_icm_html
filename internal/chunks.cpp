...
AudioID::AudioID(int track_index, std::string UID, std::string format_ID_ref, std::string pack_format_ID_ref) :
...
std::shared_ptr<Chna> Chna::read_chna_file(std::string filePath, ICM_ERROR_CODE& error_code){
    ...
    if(!chna_file.is_open()){
        ...
        while(std::getline(chna_file, chna_line)){
            ...
            if(split_line.size() == 4) pack_format_ID_ref = split_line[3];
            else pack_format_ID_ref = "";
